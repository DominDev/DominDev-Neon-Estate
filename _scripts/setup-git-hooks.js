...
if [ -n "$CSS_CHANGED" ]; then
  echo "📝 CSS files changed - auto-minifying..."
  ...
  if [ $? -eq 0 ]; then
    ...
    git add assets/css/style.min.css 2>/dev/null

    echo "✓ Minified files staged for commit"
  else
    echo "❌ CSS minification failed - commit aborted"
    exit 1
  fi
else
...
function installHook() {
  ...
  if (fs.existsSync(PRE_COMMIT_PATH)) {
    ...
    console.log('📦 Backing up existing pre-commit hook...');
  ...
  console.log('   1. You edit assets/css/style.css');
  console.log('   2. You run: git add assets/css/style.css');
  console.log('   3. You run: git commit -m "message"');
  console.log('   4. Hook automatically:');
  console.log('      - Minifies CSS files');
  console.log('      - Stages minified files (assets/css/style.min.css)');
  console.log('      - Includes them in your commit\n');

  console.log('🎯 Recommended workflow:');
  console.log('   - ALWAYS edit assets/css/style.css (source)');
  console.log('   - NEVER edit assets/css/style.min.css manually');
  console.log('   - Minification happens automatically on commit\n');

  console.log('🔄 Optional: Watch mode for development');
...
function uninstallHook() {
  console.log('🗑️  Uninstalling Git Pre-Commit Hook\n');

  if (!fs.existsSync(PRE_COMMIT_PATH)) {
    console.log('✓ No pre-commit hook found (already uninstalled)');
    return;
  }
